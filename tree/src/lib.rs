...
impl<T: Ord + Copy + Clone> RBTreeSet<T> {
    ...
    pub fn insert(&mut self, value: T) -> bool {
        ...
        while Self::balance(&mut self.root, &value) {};
    ...
    fn pattern(opt_node: &Option<Box<RBTreeNode<T>>>, value: &T) -> Option<Pattern> {
    ...
    fn balance(opt_node: &mut Option<Box<RBTreeNode<T>>>, value: &T) -> bool {
        match Self::pattern(opt_node, value) {
            Some(Pattern::LL) => {
                Self::rotate_right(opt_node);
            ...
            Some(Pattern::LR) => {
                Self::rotate_left(&mut opt_node.as_mut().unwrap().left);
                Self::rotate_right(opt_node);
                opt_node.as_mut().unwrap().color = Color::Red;
                opt_node.as_mut().unwrap().left.as_mut().unwrap().color = Color::Black;
                opt_node.as_mut().unwrap().right.as_mut().unwrap().color = Color::Black;
            ...
            Some(Pattern::RL) => {
                Self::rotate_right(&mut opt_node.as_mut().unwrap().right);
                Self::rotate_left(opt_node);
                opt_node.as_mut().unwrap().color = Color::Red;
                opt_node.as_mut().unwrap().left.as_mut().unwrap().color = Color::Black;
                opt_node.as_mut().unwrap().right.as_mut().unwrap().color = Color::Black;
            ...
            Some(Pattern::Ok) => {
                ...
                Self::balance(next_opt_node, value)
            }
            None => false,
        }
    }

    fn rotate_right(opt_node: &mut Option<Box<RBTreeNode<T>>>) {
    ...
    fn rotate_left(opt_node: &mut Option<Box<RBTreeNode<T>>>) {
        let mut a = opt_node.take().unwrap();
